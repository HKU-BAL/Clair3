...
### Option 4. Build a PyTorch environment with Mamba

**Mamba install**:

Please install Mamba using the official [guide](https://mamba.readthedocs.io/en/latest/installation.html). Then:

```bash
# ensure a bash login shell is active
source ~/.bashrc

# create and activate the PyTorch environment (CUDA 11.8 + PyTorch 2.2)
mamba env create -f env/clair3-torch.yml
mamba activate clair3-torch
```

**Install Clair3 using Mamba step by step:**
...
# clone Clair3 and compile longphase and cffi library for C implementations
cd ${HOME}
git clone https://github.com/HKU-BAL/Clair3.git
cd Clair3
make PREFIX=${CONDA_PREFIX}

# install pypy (if not available on PATH)
cd ${CONDA_PREFIX}/bin
