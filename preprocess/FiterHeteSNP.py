...
def FiterHeteSnpPhasing(args):
    ...
    vcf_fn = args.vcf_fn
    low_qual_pro = args.low_qual_pro
    ...
    if found_qual_cut_off:
        ...
        remove_low_qual_list = sorted(qual_set.items(), key=lambda x: x[1])[:int(low_qual_pro * len(qual_set))]
...
def FiterHeteSnp(args):
    ...
    alt_fn = args.alt_fn
    proportion = args.proportion
    ref_proportion = args.ref_proportion if args.ref_proportion is not None else proportion
    chr_prefix = args.chr_prefix
    contig_name = args.ctgName
    phasing_window_size = param.phasing_window_size
    ...
    if alt_fn is not None:
        ...
        low_qual_ref_list = sorted(ref_call_pos_list, key=lambda x: x[1])[:int(ref_proportion * len(ref_call_pos_list))]
        low_qual_variant_list = sorted(need_phasing_list, key=lambda x: x[1])[:int(proportion * len(need_phasing_list))]
...
def main():
    ...
    parser.add_argument('--low_qual_pro', type=float, default=0.2,
    ...
    parser.add_argument('--depth', type=int, default=1000,
        help="Default subsample depth for subsample bam file, 1000 means no subsampling. (default: %(default)s)")

    parser.add_argument('--ref_proportion', type=float, default=None,
        help="Default ref call proportion for raw alignment. (default: %(default)f)")

    parser.add_argument('--chr_prefix', type=str, default='chr',
        help="Default chr prefix for contig name. (default: %(default)s)")

    parser.add_argument('--ctgName', type=str, default=None,
        help="The name of sequence to be processed, default: %(default)s")

    parser.add_argument('--split_bed_size', type=int, default=1000,
        help="Default split bed size for parallel excution. (default: %(default)s)")
    ...
    parser.add_argument('--alt_fn', type=str, default=None,
        help="Path of the altnertive file. (default: %(default)s)")

    parser.add_argument('--realign_window_size', type=int, default=None,
        help="The window size of read realignemnt, work with need_realignment")

    parser.add_argument('--split_region_size', type=int, default=40000000,
        help="Vcf phasing split_region_size. (default: %(default)s)")
