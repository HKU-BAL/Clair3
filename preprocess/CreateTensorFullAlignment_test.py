...
class Position(object):
    def __init__(self, pos, ref_base=None, alt_base=None, read_name_list=None, base_list=None, raw_base_quality=None,
                 raw_mapping_quality=None, af=None, depth=None, genotype=None, phase_set=None, enable_dwell_time=False):
        self.pos = pos
...
def get_tensor_info(base_info, bq, ref_base, read_mq, enable_dwell_time=False):
    """
    ...
    enable_dwell_time: boolean option to decide whether enable dwell time calculation.
    """
...
def generate_tensor(ctg_name, center_pos, sorted_read_name_list, pileup_dict, ref_seq, reference_sequence,
    ...
    enable_dwell_time: boolean option to decide whether enable dwell time calculation.
    dwell_time_dict: dictionary (read name : dwell time list) for dwell time calculation.
    """
    ...
    for p in range(start_pos, end_pos):
        ...
        for read_idx, read_name_info in enumerate(sorted_read_name_list):
            ...
            if p in pileup_dict and read_name in pileup_dict[p].read_info:
                ...
                # Make a copy to avoid modifying the cached read_channel
                read_channel = list(read_channel)
                # logger.info("TEST: read_channel: %s", len(read_channel))
                if enable_dwell_time:
                    # Add dwell time with boundary check
                    if read_name in dwell_time_dict and offset < len(dwell_time_dict[read_name]):
                        # logger.info("TEST: dwell_time_dict[read_name][offset]: %d", dwell_time_dict[read_name][offset])
                        read_channel[-1] = dwell_time_dict[read_name][offset]
                    else:
    ...
    # # Fill dwell time information if enabled
    # if enable_dwell_time and dwell_time_dict is not None:
    #     dwell_time_channel_idx = 8  # The new channel for dwell time
    #     for row_idx, (hap, _, read_name) in enumerate(sorted_read_name_list):
    #         if read_name in dwell_time_dict:
    #             dwell_time_list = dwell_time_dict[read_name]
    #             # Fill dwell time for each position in the tensor
    #             # Deletion positions are already set to 0 in previous CIGAR processing
    #             for p in range(min(no_of_positions, len(dwell_time_list))):
    #                 tensor[row_idx][p][dwell_time_channel_idx] = dwell_time_list[p]
...
def _parse_mv_tag_field(mv_field):
    """
    Parse mv tag field from SAM/BAM text representation.
    ...
    if mv_field is None or not mv_field.startswith('mv:B:'):
        return None, None
    try:
        _, _, payload = mv_field.split(':', 2)
        _, values_str = payload.split(',', 1)
    except ValueError:
        return None, None
...
def compute_dwell_times(mv_field, query_sequence, is_reverse_strand):
    """
    Compute per-base dwell times from mv tag field.
    
    In nanopore sequencing, the mv tag represents per-signal-point movement:
    - 1 indicates movement to the next base position
    - 0 indicates staying at the current base position
    
    The dwell time for each base is the number of signal points spent on that base,
    which is the count of points from the previous movement to the current movement.
    
    mv_field: raw SAM tag string (e.g., "mv:B:c,5,1,0,0,1,...")
    query_sequence: read sequence string
    is_reverse_strand: boolean indicating strand orientation
...
def CreateTensorFullAlignment(args):
        ...
        for row in candidate_file_path_output:
            ...
            if len(row) > 3:  # hete snp positions
                ...
                hete_snp_pos_dict[center_pos] = Position(pos=center_pos, ref_base=ref_base, alt_base=alt_base,
                                                         genotype=int(genotype), phase_set=phase_set, enable_dwell_time=enable_dwell_time)
    ...
    debug_read_ids_fp = None
    if debug_read_ids_fn:
        debug_read_ids_fp = open(debug_read_ids_fn, 'w')
    ...
    def samtools_pileup_generator_from(samtools_mpileup_process):
        ...
        for row in samtools_mpileup_process.stdout:  # chr position N depth seq BQ read_name mapping_quality phasing_info
            ...
            #     logger.info("TEST: read_name: %s", read_name)
    ...
    # Cache of active reads in current window to reduce repeated I/O and parsing
    # key: read_name -> value: { 'pos': int, 'cigar': List[Tuple[str,int]], 'dwell_times': List[int] }
    active_read_mv_dict = {}
    ...
    while True:
        ...
        if enable_dwell_time:
            ...
            # Remove reads that are no longer active
            for stale_read in list(active_read_mv_dict.keys()):
                if stale_read not in read_name_set:
                    del active_read_mv_dict[stale_read]
            ...
            if len(missing_reads) > 0:
                ...
                samtools_view_comman = f"{samtools_execute_command} view {bam_file_path} {view_regions_option} {view_mq_option} {view_flags_option}"
                samtools_view_process = subprocess_popen(
                    shlex.split(samtools_view_comman), stdin=stdin)
                ...
                for row_id, row in enumerate(samtools_view_process.stdout):
                    ...
                    columns = row.strip().split()
                    read_name = columns[0]
                    if read_name not in missing_reads:
                        continue
                    FLAG = int(columns[1])
                    POS = int(columns[3])
                    CIGAR = columns[5]
                    ...
                    mv_tag = None
                    for c in columns[11:]:
                        if c.startswith('mv:B:'):
                            mv_tag = c
                            break
                    if mv_tag is None:
                        continue
                    dwell_times = compute_dwell_times(mv_tag, SEQ, STRAND)
                    if dwell_times is None:
                        continue
                    cigar_tuple = get_cigar_tuple(CIGAR)
                    active_read_mv_dict[read_name] = {
                        'pos': POS,
                        'cigar': cigar_tuple,
            ...
            # Build dwell_time_dict for current window using cached info
        ...
        if not unify_repre:
            ...
            if debug_read_ids_fp:
                phased_read_ids = '\n'.join([read_name for (_, _, read_name) in sorted_read_name_list])
                debug_read_ids_fp.write('\t'.join([ctg_name, str(pos), 'phased']) + '\n')
                debug_read_ids_fp.write(phased_read_ids)
                if add_no_phasing_data_training:
                    all_hap = [item[0] for item in sorted_read_name_list]
                    if sum(all_hap) != 0:
                        no_phasing_pairs = sorted([(order, read_name) for (_, order, read_name) in sorted_read_name_list])
                        no_phasing_ids = ','.join([read_name for (_, read_name) in no_phasing_pairs])
                        debug_read_ids_fp.write('\t'.join([ctg_name, str(pos), 'no_phasing', no_phasing_ids]) + '\n')
...
def main():
    ...
    parser.add_argument('--debug_read_ids_fn', type=str, default=None,
                        help="DEBUG: Output file to record the read IDs for each tensor row")
