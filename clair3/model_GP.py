...
class Clair3_F(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        x1 = tf.cast(inputs, tf.float32) / param.NORMALIZE_NUM

        x2 = self.conv1(x1)
        x3 = self.res_block1(x2)
        x4 = self.conv3(x3)
        x5 = self.res_block2(x4)
        x6 = self.conv5(x5)
        x7 = self.res_block3(x6)
        x8 = self.pyramidpolling(x7)
        x9 = self.flatten(self.L3_dropout(x8))

        x10 = self.L4(x9)
        x11 = self.L4_dropout(x10)

        l5_1_dropout = self.L5_1_dropout(self.L5_1(x11))

        l5_2_dropout = self.L5_2_dropout(self.L5_2(x11))
        ...
        if self.add_indel_length:
            ...
            l5_3_dropout = self.L5_3_dropout(self.L5_3(x10))

            l5_4_dropout = self.L5_4_dropout(self.L5_4(x10))
            ...
            if self.predict:
                ...
                # return [x2, x3, x5, x6], tf.concat([y_gt21_logits, y_genotype_logits, y_indel_length_logits_1, y_indel_length_logits_2], axis=1)
                return x7

            return [x3, x5, x6],[y_gt21_logits, y_genotype_logits, y_indel_length_logits_1, y_indel_length_logits_2]

        if self.predict:

            return [x3, x5, x6], tf.concat([y_gt21_logits, y_genotype_logits],axis=1)

        return [x3, x5, x6], [y_gt21_logits, y_genotype_logits]