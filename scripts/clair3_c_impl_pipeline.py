...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Clair3 C-impl pipeline (Python replacement for scripts/clair3_c_impl.sh)"
    )
...
def main():
    ...
    if not args.use_gpu:
        pileup_call_cmd = (
            ...
            f"-j {parallel_threads} "
            f"\"{q(args.python_bin)} {q(clair3)} CallVariantsFromCffi "
        ...
        gpu_cmd = (
            f"{q(args.python_bin)} {q(clair3)} CallVariantsFromCffiGPU "
            f"--chkpnt_fn {q(pileup_checkpoint)} "
            f"--bam_fn {q(args.bam_fn)} "
            f"--call_fn {q(pileup_vcf_path)} "
            f"--sampleName {q(args.sample_name)} "
            f"--ref_fn {q(args.ref_fn)} "
            f"--extend_bed {q(split_bed_path)} "
            f"--bed_fn {q(args.bed_fn)} "
            f"--vcf_fn {q(args.vcf_fn)} "
            f"--platform {q(args.platform)} "
            f"--fast_mode {q(args.fast_mode)} "
    ...
    if not args.use_gpu:
        full_aln_call_cmd = (
            ...
            f"--minMQ {q(args.min_mq)} "
            f"--minCoverage {q(args.min_coverage)} "
            f"--phased_vcf_fn {q(phase_vcf_path / 'phased_{/.}.vcf.gz')} "
            f"--gvcf {q(args.gvcf)} "
            f"--enable_long_indel {q(args.enable_long_indel)} "
            f"--samtools {q(args.samtools)} "
            f"--use_gpu {q(args.use_gpu)} "
        ...
        gpu_cmd = (
            ...
            f"--chkpnt_fn {q(full_alignment_checkpoint)} "
            f"--bam_fn {q(args.bam_fn)} "
            f"--call_fn {q(full_alignment_output_path)} "
            f"--sampleName {q(args.sample_name)} "
            f"--vcf_fn {q(args.vcf_fn)} "
            f"--ref_fn {q(args.ref_fn)} "
            f"--add_indel_length "
            ...
            f"--minMQ {q(args.min_mq)} "
            f"--minCoverage {q(args.min_coverage)} "
            f"--phased_vcf_fn {q(phase_vcf_path)} "
            f"--gvcf {q(args.gvcf)} "
            f"--enable_long_indel {q(args.enable_long_indel)} "
            f"--samtools {q(args.samtools)} "
            f"--use_gpu {q(args.use_gpu)} "
            ...
            f"--internal_threads {q(internal_threads)} "
            f"--device {q(args.device)} "
            f"--full_aln_files {q(full_aln_files_path)}"
        )
        run_command(gpu_cmd, log_path=log_path / "6_call_var_bam_full_alignment.log")
